...
def snip(model, rm_modules, dataloader, device):
    ...
    model.zero_grad()
    global INPUT_DATA
    INPUT_DATA = inputs.clone().detach().cpu()
    return score

def synflow(model, rm_modules, dataloader, device):
    device = [i + 1 for i in range(device - 1)]
    ...
    for i, (inputs, targets) in enumerate(dataloader):
        inputs = torch.ones([1] + list(inputs[0].size()))
        inputs, targets = inputs.cuda(1), targets.cuda(1)
        outputs = model(inputs)
...
def ReFer(model, rm_modules, dataloader, device):
    device = [i + 1 for i in range(device - 1)]

    global loss
    def store_feature(module, input, output):
        global loss
        loss += output.abs().sum().to('cuda:1')
