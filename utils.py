...
def get_dataloader(name, batch_size, data_download=True):
    ...
    if name == 'cifar10':
        ...
        dataset_dir = '/mnt/tiny-imagenet-200/'
        ...
        dataset_dir = '/mnt/datasets/ImageNet1k/ILSVRC2012/'
        train_dir = os.path.join(dataset_dir, 'train')
        val_dir = os.path.join(dataset_dir, 'val')
    ...
    return torch.utils.data.DataLoader(train_data, batch_size=batch_size, shuffle=True, num_workers=4), torch.utils.data.DataLoader(valid_data, batch_size=batch_size, shuffle=True, num_workers=4),torch.utils.data.DataLoader(train_data, batch_size=int(batch_size/2),shuffle=True, num_workers=4),torch.utils.data.DataLoader(valid_data, batch_size=int(batch_size/2),shuffle=False, num_workers=4)
...
def train_multi_gpu(model, train_loader, loss_func, optimizer, epoch, mixup=False):
    if mixup:
        mixup_fn = Mixup(**mixup_args)
